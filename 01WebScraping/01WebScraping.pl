...
use Mojo::UserAgent;
#---------------------------------------------------------------Records keeping
my $APP_NAME = "Perl/Tk Web Scraping";
my $VERSION = " V0.1";
...
my $filename = "https://www.vremea.com/vremea-bucuresti/LRBS";
# -----------------------------------------------------------------------Window 
$mw = MainWindow->new;
...
$f->Button(-text => "Save", -command => \&save_file)->
  pack(-side => 'right', -anchor => 'e');
$f->Button(-text => "Load", -command => \&loadSite)->
  pack(-side => 'right', -anchor => 'e');
...
my $res = $ua ->get($filename)->result;

if    ($res->is_success)  { say $res->body; $t->insert('end', $res->body); }
